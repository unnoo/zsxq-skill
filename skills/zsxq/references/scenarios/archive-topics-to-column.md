...
把某星球最新的 N 条主题批量收录进指定专栏，适合整理专题合集、活动精华归档、定期把优质新内容归入常设专栏。**收录是写入操作：先列出目标专栏与待收录主题清单供用户确认，确认后再逐条执行，每次请求间隔约 2 秒避免限速。**
...
- 只收录单条主题 → 直接用 [column-add-topic](../column-add-topic.md) 原子操作，无需场景
...
| 收录主题到专栏 | [column-add-topic](../column-add-topic.md) | 逐条把主题收录进目标专栏 |
...
用户确认后，对每个 `topic_id` 调用 [column-add-topic](../column-add-topic.md) 收录，**每次请求之间间隔约 2 秒**避免触发限速。逐条记录结果，全部完成后按顺序报告每条的 ✅ 成功 / ❌ 失败（失败附原因）。
...
- 每条收录是独立请求，单条失败不影响其他主题；记录失败的 `topic_id` 与原因，可对失败项单独重试
- 达到 100 上限后，后续条目会持续失败 → 停止收录并如实报告已收录数量与未收录清单
- 若主题已在专栏中，重复收录的行为以接口返回为准（规格未说明是否报错），重试前可先用 `getgroupcolumntopics` 或专栏主题列表核对该主题是否已被收录
- 本场景为增量写入、无整体事务；中断后可从未收录清单续跑

## 附加资源

- [column-add-topic](../column-add-topic.md) — 收录主题的原子操作（含 CAUTION / 100 条上限）
